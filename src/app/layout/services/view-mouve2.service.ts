...
export class ViewMouve2Service {

  private baseUrl = globals.apiBaseUrl + 'viewMouve2';
  ...
  getCA2Client(op: string): Observable<string> {
    return this.http.get<string>(`${this.baseUrl}/search/CA2Client?op=${op}`);
   }

   getMA2Client(op: string): Observable<string> {
    return this.http.get<string>(`${this.baseUrl}/search/MA2Client?op=${op}`);
   }

 
   getEvolutionCA(client: string, article: string, fournisseur: string, typologieClient: string, famille: string, 
    vendeur: string, secteur: string, consRev: string, 
