...
@Component({
  selector: 'app-nouvelle-offre-rg-mg',
  templateUrl: './nouvelle-offre-rg-mg.component.html',
  styleUrls: ['./nouvelle-offre-rg-mg.component.scss'],
  providers: [ToolbarService, ResizeService, ConfirmationService, DatePipe],
  encapsulation: ViewEncapsulation.None,
})
export class NouvelleOffreRgMgComponent implements OnInit, AfterViewChecked {
  ...
  async ngOnInit() {
    await this.getNumeroDevis();
    await this.devisService.update(this.V2).toPromise().then();
    this.visible = false;
    this.visible2 = false;
  ...
  verifClt() {
    this.ngSelectVendeur.focus();
  }

  verifVnd() {
    this.mouveToNext(`refClt`);
  }

  verifRef() {
    this.mouveToNext(`refDmPr`);
  }
  ...
  async validationCmd(e) {
    ...
    edevis.datDev = this.dated.toLocaleDateString();
    ...
    edevis.remGen = this.remiseGlobale;
    edevis.dateEnvoi = this.dated.toLocaleDateString();
    edevis.basPage = this.basDePage;
    this.responsable
      ? (edevis.attention = String(this.responsable).substr(0, 20))
      : (edevis.attention = '');
    if (
      ...
      client.deno = this.m;
      client.numDev = this.V2;
      client.adresse = this.adresse;
      client.ville = this.ville;
      client.codeTva = this.codeTva;
      console.log(client);
      await this.nomClientOffreService
        .createNomClient(client)
        .toPromise()
        .then();
    }
    console.log(edevis);

    await this.edevisService.createEdevis(edevis).toPromise().then();
  ...
  async verifPrix(prix, rawData, e, id) {
    ...
    if (prix !== null && prix !== undefined && Number(prix) > 0) {
      ...
      /*rawData.prix = (
        ((10 * Number(rawData.prixInit) * (100 - this.remiseGlobale)) +
        0.49) / 1000
      ).toFixed(3);*/
      console.log(rawData, this.remiseGlobale);
    } else {
      this.affichValider = true;
      this.ajout = true;
      this.msg = 'veuillez donner le prix de cet article';
    ...
    if (this.selectedStock.length === 0) {
      ...
      if (index === -1) {
        ...
        /*setTimeout(() => {
          ...
          document.getElementById(`row_${art.id}_inputQte`).focus();
          document.getElementById(`row_${art.id}_inputQte`).click();
        }, 100);*/
      }
      setTimeout(() => {
        this.mouveToNext(`row_${art.id}_inputQte`);
  ...
  async rechArticle(mot) {
    ...
    /*if (this.listeStocks.length > 0) {
      setTimeout(() => this.gridstock.selectRows([0]), this.listeStocks.length + 5);
    }*/
  }
  async EtabCMD(e) {
    this.wasInside = true;
    ...
    if (this.selectedClient !== null && this.selectedClient !== undefined) {
      if (this.m === null || this.m === undefined || this.m === '') {
        ...
        if (
          ...
          this.etab_cmd = true;
          this.basDePage =
            'CONDITIONS COMMERCIALES : \nDelai de Livraison:  Disponible sauf vente entre temps. \nMode de paiement: ' +
            this.selectedClient.reg +
            '\nValidite de l\'offre  : 30 jours.\nMode de Livraison: Par vos soins.';
          this.rechArticle('');
        } else {
  ...
  async ctrlClt(client) {
    await this.brouService
      .controleClient(client)
      .toPromise()
      .then((data) => {
        this.controle = data['_embedded'].controleClients[0];
        this.controle.solde = Number(this.controle.solde).toFixed(3);
        this.controle.imp = Number(this.controle.imp).toFixed(3);
        this.controle.retard = Number(this.controle.retard).toFixed(3);
      });
    if (
      Number(this.selectedClient.plafond) < Number(this.controle.solde) ||
      Number(this.controle.imp) > 0 ||
      Number(this.controle.retard) > 0
    ) {
      this.controleMsg = 'ATTENTION CE CLIENT A DES PROBLEMES';
    }
    if (Number(this.selectedClient.plafond) < Number(this.controle.solde)) {
      this.controleMsg = this.controleMsg.concat(
        '<br>SOLDE ' +
          this.controle.solde +
          ' PLAFOND ' +
          this.selectedClient.plafond
      );
    }
    if (Number(this.controle.imp) > 0) {
      this.controleMsg = this.controleMsg.concat(
        '<br>IMPAYE ' + this.controle.imp
      );
    }
    if (Number(this.controle.retard) > 0) {
      this.controleMsg = this.controleMsg.concat(
        '<br>RETARD ' + this.controle.retard
      );
    }
    if (this.controleMsg && this.controleMsg !== '') {
      await this.confirmationService.confirm({
        message: this.controleMsg,
        header: 'Avertissement',
        icon: 'pi pi-exclamation-triangle',
        acceptLabel: 'OK',
        rejectVisible: false,
  ...
  async changeSelectedItem() {
    if (this.selectedClient !== null && this.selectedClient !== undefined) {
      ...
      this.selectedClient.respon
        ? (this.responsable = 'Mr. ' + String(this.selectedClient.respon).substr(0, 16))
        : (this.responsable = 'Mr.');
      if (
      ...
      if (this.codeClient.startsWith('411')) {
        this.controleMsg = '';
        this.ctrlClt(this.selectedClient.code);
      }
      await this.edevisService
  ...
  async getNumeroDevis() {
    await this.devisService
      .getNumero()
      .toPromise()
      .then((data) => {
        this.devis = data['_embedded'].devis;
        this.V2 = this.devis[0].numero;
        this.V1 = +this.V2 + 1;
  ...
  isNull(chaine: any): string {
    if (chaine === null || String(chaine) === 'null') {
      return '';
    } else {
      return chaine;
    }
  ...
  async imprimer(evenement) {
    ...
    if (entete) {
      if (societe === 'EQUIPEMENT MODERNE F.INDUSTRIE') {
        ...
        doc1.text('ADRESSE..:  ' + this.isNull(societe.adresse), 9, 20);
        doc1.text('VILLE....:  ' + this.isNull(societe.ville), 9, 25);
        doc1.text('TEL......:  ' + this.isNull(societe.tel), 9, 30);
        doc1.text('FAX......:  ' + this.isNull(societe.fax), 9, 35);
        doc1.text('E-MAIL...:  ' + this.isNull(societe.email), 9, 40);
      }
    }
    doc1.text(
      'Tunis, le : ' + this.datepipe.transform(this.dated, 'dd/MM/yyyy'),
      165,
      50
    );
    doc1.setFontSize(24);
    doc1.setFontStyle('bold');
    doc1.text('Offre de Prix N°:' + this.V2, 63, 50);
    doc1.setFontSize(12);
    doc1.text(
    ...
    this.notreRef =
      ...
      this.V1 +
      '/' +
      this.datepipe.transform(this.dated, 'yy');
    ...
    doc1.text('TEL: ' + this.isNull(this.tel) + ' FAX: ' + this.isNull(this.fax), 135, 98);
